...
### Requirement: Cambiar el precio de un plan no modifica pagos ya registrados
Agregar un nuevo precio a un plan SHALL NOT modificar el monto pagado ni el precio de referencia
de ningún pago registrado antes de ese cambio.

#### Scenario: Pago histórico no cambia tras un nuevo precio del plan
...
- **WHEN** se cambia el plan de un miembro después de haberle registrado un pago con el plan
  anterior
- **THEN** ese pago sigue mostrando en el listado el plan y precio con los que se registró
  originalmente, no el plan nuevo del miembro
