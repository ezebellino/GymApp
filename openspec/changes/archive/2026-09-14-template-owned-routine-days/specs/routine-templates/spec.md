...
**Reason**: Reemplazado por la composición de ejercicios propios de cada día de plantilla,
agregados y quitados mediante un buscador sobre el catálogo activo (ver "Alta y baja de ejercicios
en un día de plantilla mediante búsqueda" en este mismo spec), en lugar de activar/desactivar un
ejercicio del catálogo global de días. El modelo de "activo/inactivo por plantilla" sobre un
catálogo de días compartido deja de existir.
**Migration**: Migración de esquema sin backfill — la configuración existente de días y
ejercicios (`TrainingDay`, `TrainingDayExercise`) se descarta; se recrea a mano o vía seed de
desarrollo.

### Requirement: Base del ejercicio al crear o editar en el catálogo
**Reason**: El catálogo de ejercicios deja de tener una base (series × reps · kg) propia editable.
La base pasa a ser exclusiva de cada combinación (plantilla, día, ejercicio): al agregar un
ejercicio a un día de plantilla, arranca en un valor fijo por defecto de 3 series × 10
repeticiones × 0 kg (ya no copiado de una base del catálogo), y se ajusta desde ahí sin afectar a
ninguna otra plantilla (ver "Alta y baja de ejercicios en un día de plantilla mediante búsqueda").
**Migration**: Migración de esquema sin backfill — se retiran los campos de base del catálogo de
ejercicios; no hay valores que preservar porque las plantillas y días existentes también se
descartan en este mismo change.
