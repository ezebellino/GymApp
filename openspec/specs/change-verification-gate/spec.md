...
Gate de verificación de un change de OpenSpec, detrás de `/opsx:verify` (skill `verify-change`):
un paso 0 mecánico (`make check-plan`, `make lint`, `make test`) que filtra antes de gastar un
turno de agente, un `## Plan de verificación` obligatorio en el `design.md` de todo change con
código (invariantes, tests nombrados por archivo y caso, riesgo declarado), y una política de QA
manual gobernada por ese riesgo: bajo la omite y lo registra, medio la corre, alto nunca la
saltea. Existe para reducir las pasadas de verificación y el retrabajo que producían hallazgos
declarables antes de escribir código.
